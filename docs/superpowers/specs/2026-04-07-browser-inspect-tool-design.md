...
|---|---|---|---|---|
| NEW-1 | 공통 컴포넌트 잘못된 파일 수정: 사용자가 Page A의 Button을 클릭 → 에이전트가 공유 Button.tsx 전체 수정 → Page B, C, D 망가짐 | High | **MVP**: agent prompt 규약만 (Candidate 2 checklist에 "공통 컴포넌트 수정 전 `grep -c` import 횟수 확인" 단계 추가 — 옵션 B). **Post-MVP**: 운영 데이터에서 prompt 규약 우회 사례 발견 시 `caller_chain` payload 필드 추가 (옵션 A). 처음부터 둘 다 빌드 안 함 (over-engineering 회피) | **decided: B-start → A-reinforce-post-MVP** (2026-04-08) |
