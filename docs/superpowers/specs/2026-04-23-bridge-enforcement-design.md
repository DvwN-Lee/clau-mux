...
4. **npx cache behavior (P2 operational)**: pin `clau-mux-bridge@^1.3.0` is cached by `npx` in `~/.npm/_npx` on first resolution. A later publish of `1.3.1` matching the same semver range will NOT be auto-fetched — npx reuses the cached `1.3.0`. To distribute a critical hotfix within the `1.3.x` range, maintainer must either (a) bump setup scripts to an exact version like `clau-mux-bridge@1.3.1`, (b) document `npm cache clean --force` as a workaround in the release notes, or (c) switch from `^1.3.0` to `@latest` and accept weaker pin semantics. Acceptable for current scope because `1.3.x` is a fresh line; revisit if a security-critical `1.3.y` fix ever ships.
