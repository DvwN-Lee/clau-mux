...
You are `gemini-worker` (or the specific name assigned via `-n`). This document describes a team-mode protocol that applies **only when you are attached to a multi-agent team** managed by Claude Code.
