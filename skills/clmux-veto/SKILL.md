...
| **critic** (Codex 또는 Gemini Pro) | 논리 결함·가정 검증 | rebuttal, secondary critique |
...
| 항목 | Lead (anthropic) | clmux teammate (비-Claude) |
|---|---|---|
| 투표권 (Approve/VETO) | X (중재자) | **O** (provider_family 단위 독립 투표) |
| Evidence Pack 기반 의견 | X (중재) | **O** (Rule 3 판정 근거) |
| 독립 지지 인정 | — (Anthropic teammate 미운영) | 각 provider_family 1개 |

VETO 투표는 clmux teammate 전원이 provider_family 단위로 수행한다 — Cross-Provider 합의 모델.
Lead는 투표에 참여하지 않고, Evidence Pack 기반 합의 결과를 중재·판정한다 (Anchoring bias 방지).
Rule 3은 본질적으로 보장됨 — Anthropic teammate 0명, 모든 투표가 비-Claude provider_family. 단 단일 provider 운영 시 Cross-Provider 다양성 미충족 → 판정 로그에 명시 + Human 검토 권장.
...
| **Low** | CI Green + Copilot PR 리뷰 (manual 실행, Pro 티어) 통과 → VETO 생략[^veto-low-copilot] |

[^veto-low-copilot]: 본 팀은 Copilot Pro Individual ($10/mo) 에서 **manual PR 리뷰**로 사용 (org-level auto-review 는 Business/Enterprise 한정으로 접근 불가). 통과 근거는 Copilot Code Review 품질 baseline — 2025-04~2026-03 기간 60M 리뷰 처리 중 71% actionable / 29% silent (GitHub Blog "60 million Copilot code reviews and counting", 2026-03-05). 코드 품질 blind peer-review 연구 (n=243 Python 개발자, 25 senior reviewer 의 1,293 리뷰, Nov 2024 GitHub 후원 외부 수행): Copilot 보조 코드의 unit test 전체 통과 확률 +53.2% (p<0.01), 리뷰어 승인률 +5% (p=0.014). 해당 품질 지표가 하락하거나 tier 가 downgrade 되면 본 gate 재심.
...
1. 토론 완료 후 각 Teammate는 `votes/{agenda-id}-{teammate-name}.md` 파일 작성:
   ...
   # VOTE: {Approve|VETO}
   ## Teammate: {name}
   ## provider_family: {google|openai|github}
   ## 안건: {agenda-id}
   ## Evidence Pack 기반 근거
...
3. Lead가 `votes/` 디렉토리를 일괄 읽고 Phase별 합의 기준에 따라 판정:
   - **P1**: VETO가 1건이라도 있으면 해당 사유 토론 → 수정 후 재투표
     - Lead = Sonnet인 경우: 검증 Subagent(`model="sonnet"`) 독립 검토 병행 — clmux-phase §Lead=Sonnet 역할 보완 참조.
...
- Phase 3 Generator가 Codex(openai)면, Phase 4 Validator는 Gemini(google) 또는 Copilot(github) 등 다른 provider_family로 배정
- Phase 3 Generator가 Lead Subagent(`model="sonnet"`, anthropic)면, Phase 4 Validator는 비-Claude teammate ≥1개 독립 검증 필수
