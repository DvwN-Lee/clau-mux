...
# Inline project detection (replaces detect-project.sh dependency)
_detect_cwd="${CLAUDE_PROJECT_DIR:-$(pwd)}"
PROJECT_ROOT=$("$PYTHON_CMD" -c "
...
try:
    ...
    print(cwd)
" 2>/dev/null || echo "$_detect_cwd")

PROJECT_NAME=$(basename "$PROJECT_ROOT")

PROJECT_ID=$("$PYTHON_CMD" -c "
import subprocess, hashlib
root = '$PROJECT_ROOT'
try:
    r = subprocess.run(['git', '-C', root, 'remote', 'get-url', 'origin'],
                       capture_output=True, text=True, timeout=5)
    src = r.stdout.strip() if r.returncode == 0 and r.stdout.strip() else root
except:
    src = root
print(hashlib.sha256(src.encode()).hexdigest()[:12])
" 2>/dev/null || echo "global")

PROJECT_DIR="${HOME}/.claude/homunculus/projects/${PROJECT_ID}"
...
if parsed["output"] is not None:
    SEARCH_TOOLS = {"Grep", "Glob", "Search", "WebSearch"}
    if parsed["tool"] in SEARCH_TOOLS:
