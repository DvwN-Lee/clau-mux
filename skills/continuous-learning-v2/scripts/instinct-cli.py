...
def _load_jsonl(filepath: Path) -> list[dict]:
    """Load a JSONL file, skipping malformed lines. Returns [] if file absent."""
    rows = []
...
def cmd_diagnose(args) -> int:
    """Analyse problems.jsonl and report detected inefficiency patterns.

    Shows session counts, problem type distribution, and per-type details.
    """
    project = detect_project()
    obs_file = project.get("observations_file")
    project_dir = Path(str(obs_file)).parent if obs_file else HOMUNCULUS_DIR

    problems_path = project_dir / "problems.jsonl"
    problems = _load_jsonl(problems_path)
    ...
    if not problems:
        print(f"  No problems.jsonl found.")
        print(f"  Path: {problems_path}")
        print(f"  Run with ECC_ENABLE_PROBLEM_DETECTOR=1 to start collecting data.")
        print(f"\n{'='*60}\n")
        return 0

    total_records = len(problems)
    unique_sessions = len(set(p.get("session_id") for p in problems if p.get("session_id")))
    problematic = [p for p in problems if p.get("summary", {}).get("has_problems")]
    clean_records = total_records - len(problematic)

    problem_type_counts: dict = defaultdict(int)
    for p in problematic:
        for pt, det in (p.get("problems") or {}).items():
            if isinstance(det, dict) and det.get("is_inefficient"):
                problem_type_counts[pt] += 1

    print(f"  Records: {total_records} total ({clean_records} clean, {len(problematic)} with problems)")
    print(f"  Sessions: {unique_sessions} unique")

    if problematic:
        print(f"\n  Problem type breakdown:")
        for pt, cnt in sorted(problem_type_counts.items(), key=lambda x: -x[1]):
            pct = round(cnt / len(problematic) * 100)
            print(f"    {pt}: {cnt} sessions ({pct}%)")

    # Average inefficiency score
    scores = [p.get("summary", {}).get("inefficiency_score", 0) for p in problems]
    avg_score = sum(scores) / len(scores) if scores else 0
    print(f"\n  Average inefficiency score: {avg_score:.3f}")
...
def main() -> int:
    ...
    # Diagnose (new in v2.2)
    diagnose_parser = subparsers.add_parser(
        'diagnose',
        help='Report detected inefficiency patterns from problems.jsonl',
    )
