---
name: skills-audit
description: "Use when auditing Claude skills and commands for quality. Supports Quick Scan (changed skills only) and Full Audit modes with sequential subagent batch evaluation."
origin: ECC
---

# skills-audit

Slash command (`/skills-audit`) that audits all Claude skills and commands using a quality checklist + AI holistic judgment. Supports two modes: Quick Scan for recently changed skills, and Full Audit for a complete review.
...
Run: `bash ~/.claude/skills/skills-audit/scripts/scan.sh`
...
| Skill | 7d use | 30d use | Problems | Confidence | Description |
|-------|--------|---------|----------|------------|-------------|
...
{ \"verdict\": \"Keep\"|\"Improve\"|\"Patch\"|\"Update\"|\"Retire\"|\"Merge into [X]\", \"reason\": \"...\" }
"
)
```
...
- [ ] Problem data checked (problems.jsonl, if available)
```
...
| Patch | Skill content directly caused a detected agent inefficiency (evidenced by problems.jsonl data); requires targeted fix |
...
Phase 1 scan (`scan.sh`) automatically loads `problems.jsonl` from the same project data directory as `observations.jsonl` (auto-detected via git+sha256 hash). If `problems.jsonl` does not exist, the scan silently falls back to zero-valued defaults — existing behavior is unaffected.
