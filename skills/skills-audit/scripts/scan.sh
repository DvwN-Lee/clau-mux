...
# Auto-detects continuous-learning-v2 project-scoped path via git+sha256 (same logic as detect-project.sh).
# Override via SKILL_STOCKTAKE_OBSERVATIONS env var if your setup uses a different path.
if [[ -n "${SKILL_STOCKTAKE_OBSERVATIONS:-}" ]]; then
  OBSERVATIONS="$SKILL_STOCKTAKE_OBSERVATIONS"
else
  _scan_proj_root="${CWD_SKILLS_DIR%/.claude/skills}"
  _scan_remote=$(git -C "$_scan_proj_root" remote get-url origin 2>/dev/null \
    | sed -E 's|://[^@]+@|://|' || true)
  _scan_hash_src="${_scan_remote:-$(git -C "$_scan_proj_root" rev-parse --show-toplevel 2>/dev/null || true)}"
  _scan_pid=""
  if [[ -n "$_scan_hash_src" ]] && command -v python3 >/dev/null 2>&1; then
    _scan_pid=$(printf '%s' "$_scan_hash_src" \
      | python3 -c "import sys,hashlib; print(hashlib.sha256(sys.stdin.buffer.read()).hexdigest()[:12])" \
      2>/dev/null || true)
  fi
  if [[ -n "$_scan_pid" ]]; then
    OBSERVATIONS="$HOME/.claude/homunculus/projects/$_scan_pid/observations.jsonl"
  else
    OBSERVATIONS="$HOME/.claude/observations.jsonl"
  fi
fi
...
count_obs() {
  ...
  jq -r --arg p "$file" --arg c "$cutoff" \
    'select(.tool=="Read" and .timestamp>=$c) |
     select(.input != null) |
     .input | try fromjson | select(type == "object") |
     .file_path | select(. == $p) | 1' \
...
scan_dir_to_json() {
  ...
  # Pre-aggregate observation counts in two jq passes on tail file.
  # Reads only the tail of observations.jsonl (last 2MB) for performance,
  # then emits 7d and 30d counts together to avoid scanning the file twice.
  local obs_7d_counts obs_30d_counts _obs_tail_file
  obs_7d_counts=""
  obs_30d_counts=""
  if [[ -f "$OBSERVATIONS" ]]; then
    _obs_tail_file="$tmpdir/_obs_tail.jsonl"
    # Read last 2MB only (current data is at the end of the append-only file)
    tail -c 2097152 "$OBSERVATIONS" 2>/dev/null \
      | sed '1{/^[^{]/d;}' > "$_obs_tail_file"
    # Single jq pass: emit "7d <path>" or "30d <path>" tags
    local _obs_combined
    _obs_combined=$(jq -r --arg c7 "$c7" --arg c30 "$c30" \
      'select(.tool=="Read" and .timestamp>=$c30) |
       select(.input != null) |
       .input | try fromjson | select(type == "object") |
       .file_path // empty |
       select(. != "") |
       if . then . else empty end' \
      "$_obs_tail_file" 2>/dev/null | sort | uniq -c)
    # For 30d: all lines from the combined output
    obs_30d_counts="$_obs_combined"
    # For 7d: re-filter from tail with 7d cutoff only
